...
% name and index of each region in the DCM
nregions    = 3; 
VOI_names = {'PPA_r' 'HC_ant_r' 'PreCun_r' 'rACC_r'};
PPA_r=1; HC_ant_r=2; PreCun_r=3; rACC_r=4;
for voi = 1:nregions % rACC not used here
    eval(sprintf('%s = %d;', VOI_names{voi}, voi));
end
...
% A-matrix (on / off)
a = ones(nregions,nregions); % assume full intrinsic connectivity
% for 4 regions, rACC-PPA connectivity might not be reasonable to assume
